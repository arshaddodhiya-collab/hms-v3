...
```typescript
const commands = [
  { id: 'dashboard', phrases: ['Dashboard', 'Home', 'Dash board'] },
];
const result = fuse.search(spokenText);
if (result[0].score < 0.4) {
  // 0.4 = 40% difference tolerance
...
The app should know _where_ it is.
...
- This restricts the Fuzzy Matcher to _only_ look for relevant commands, drastically reducing error rates.
...
### Backend (Node/NestJS) - _Required for "Any Situation"_
